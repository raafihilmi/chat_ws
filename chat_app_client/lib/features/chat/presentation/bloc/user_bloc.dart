...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  Future<void> _onBlockUserEvent(
    ...
    result.fold(
      ...
      (block) => emit(UserActionSuccess(message: 'User blocked successfully')),
    );
  }
  ...
  Future<void> _onReportUserEvent(
    ...
    emit(UserLoading());
    final result =
        await reportUser(ReportUserParams(reason:event.reason,reportedUserId: event.reportUserId));
    result.fold(
        (failure) =>
            emit(UserError(message: 'Failed to report user: $failure')),
        (report) => emit(UserActionSuccess(message: 'User reported')));
  }
}
